...
pub fn load_entries(conn: &Connection, query: &EntryQuery) -> Result<Vec<EntryRow>> {
    ...
    let rows = stmt.query_map(params_from_iter(params.iter()), |row| {
        Ok(EntryRow {
            ...
            request_cookies: row.get(9)?,
            request_body_hash: row.get(10)?,
            request_body_size: row.get(11)?,
            status: row.get(12)?,
            status_text: row.get(13)?,
            response_headers: row.get(14)?,
            response_cookies: row.get(15)?,
            response_body_hash: row.get(16)?,
            response_body_size: row.get(17)?,
            response_body_hash_raw: row.get(18)?,
            response_body_size_raw: row.get(19)?,
