...
pub fn load_entries(conn: &Connection, query: &EntryQuery) -> Result<Vec<EntryRow>> {
    ...
    let rows = stmt.query_map(params_from_iter(params.iter()), |row| {
        Ok(EntryRow {
            ...
            url: row.get(12)?,
            host: row.get(13)?,
            http_version: row.get(14)?,
            request_headers: row.get(15)?,
            request_cookies: row.get(16)?,
            request_body_hash: row.get(17)?,
            request_body_size: row.get(18)?,
            status: row.get(19)?,
            status_text: row.get(20)?,
            response_headers: row.get(21)?,
            response_cookies: row.get(22)?,
            response_body_hash: row.get(23)?,
            response_body_size: row.get(24)?,
            response_body_hash_raw: row.get(25)?,
            response_body_size_raw: row.get(26)?,
            response_mime_type: row.get(27)?,
            server_ip: row.get(28)?,
            connection_id: row.get(29)?,
            tls_version: row.get(30)?,
            tls_cipher_suite: row.get(31)?,
            tls_cert_subject: row.get(32)?,
            tls_cert_issuer: row.get(33)?,
            tls_cert_expiry: row.get(34)?,
            entry_extensions: row.get(35)?,
            request_extensions: row.get(36)?,
            response_extensions: row.get(37)?,
            content_extensions: row.get(38)?,
            timings_extensions: row.get(39)?,
            post_data_extensions: row.get(40)?,
        })
    })?;
